...
"""
def multi_plots(filename, data, titles, fs, plot_rows, win_len, win_hop, step=1, colors=["b", "r", "m","g"]):
...
def predict_gender(F0_mean): 
    F0_mean = mean
    if abs(F0_mean - 114) < abs(F0_mean - 195):
        return 'Male'
    else:
        return 'Female'
    '''
...
if __name__ == "__main__":
    # tạo tên file
    froot = "E:\DUT\Kì_5\Xử lí THS\Tín hiệu mẫu\\"
    filename = ["MHB04", "MVB05", "FTC07", "MLD08", "MPD09", "MSD10", "FTD12", "FHH14", "FTH16", "FTL24"]
    threshold = []
    win_len = 0.03
    win_hop = 0.015
    F0_mean_Cepstrum = []
    ...
    for i in range(len(filename)):
        ...
        print("Cepstrum:")
        m, sd = F0_mean_std(y_values)
        fm = predict_gender(0)
        
        F0_mean_Cepstrum.append(m)
    
        
        #print("FFT findpeaks:")
        m, sd = F0_mean_std(y_another)
        fm = predict_gender(0)
        F0_mean_FFT.append(m)
        F_M.append(fm)
        ...
        multi_plots(fig_name, data=[sig, energy, vad, index_real],
                titles=["Input signal (voiced + silence)", "Short time energy",
                        "Voice activity detection", "Ouput signal (voice marked)",
                        "Cepstrum algorithm", "Findpeaks FFT algorithm"],
                fs=fs, plot_rows=6, win_len=win_len, win_hop=win_hop, step=1)
        F_M_lab.append(read_fm(filename[i]))
    d = {'Filename': filename, 'Lab F/M': F_M_lab, 'Findpeaks M/F': F_M, 'F0_mean_FFT': np.round(F0_mean_FFT, 2), 'F0_mean_Cepstrum': np.round(F0_mean_Cepstrum, 2)}
    df = pd.DataFrame(data=d)
    print(df)
    print("Accuracy: ", accuracy(F_M_lab, F_M), "%")
